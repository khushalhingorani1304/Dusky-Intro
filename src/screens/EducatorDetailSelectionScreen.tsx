...
export interface EducatorDetailOption {
  id: string;
  label?: string;
  description?: string;
...
interface EducatorDetailSelectionScreenProps {
  title: string;
  subtitle?: string;
  options: EducatorDetailOption[];
  onBack: () => void;
...
const EducatorDetailSelectionScreen: React.FC<EducatorDetailSelectionScreenProps> = ({
  title,
  subtitle,
  options,
  onBack,
  ...
  const handleSelect = (option: EducatorDetailOption) => {
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center bg-[#F8F7FF] py-20 px-6">
      <div className="w-full max-w-5xl rounded-[36px] border border-white/40 bg-white px-10 py-14 shadow-[0_28px_60px_-32px_rgba(63,55,146,0.32)]">
       ...
       <div className="mt-12 flex flex-wrap gap-6 md:justify-between lg:justify-center lg:gap-10">
...
export default EducatorDetailSelectionScreen;
